...
import { Camera, Loader2, X, Tag, Banknote, Sparkles, Zap, SlidersHorizontal, Check, Type, Phone } from "lucide-react";
...
export default function PostEditor() {
  ...
  // Fonction wrapper pour capturer la miniature avant l'upload
  const handleUpload = async (files: File[]) => {
    const filesWithThumbnails = await Promise.all(files.map(async (file) => {
      if (file.type.startsWith("video/")) {
        ...
        if (blob) {
           // On attache la miniature au fichier (via une propriété personnalisée)
           (file as any).thumbnail = new File([blob], "thumb.jpg", { type: "image/jpeg" });
        }
      }
      return file;
    }));
    startUpload(filesWithThumbnails);
  };
    ...
    mutation.mutate({
      ...
      },
      {
        onSuccess: () => {
          ...
          setProductName(""); setPrice(""); setPhone(""); setStock("1"); setTargetUserId("me");
          resetMediaUploads();
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* ... (Reste de votre UI inchangée jusqu'aux attachments) */}
      <div className="flex items-center justify-between pb-4 border-b border-border/40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t.seller_studio}</p>
          </div>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <input placeholder={t.product_name} value={productName} onChange={(e) => setProductName(e.target.value)} className="w-full h-12 pl-12 rounded-2xl bg-[#f8faff] border border-[#4a90e2]/10 text-sm font-black uppercase" />
        <input placeholder={t.price} type="number" value={price} onChange={(e) => setPrice(e.target.value)} className="w-full h-12 pl-12 rounded-2xl bg-[#f8fff8] border border-[#6ab344]/10 text-sm font-black text-[#6ab344]" />
        <input placeholder={t.whatsapp_number} type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full h-12 pl-12 rounded-2xl bg-[#f8faff] border border-[#4a90e2]/10 text-sm font-semibold" />
      </div>

      <div className="relative max-w-[160px]">
        <input placeholder={t.stock} type="number" min="0" value={stock} onChange={(e) => setStock(e.target.value)} className="w-full h-12 px-4 rounded-2xl bg-[#f8faff] border border-[#4a90e2]/10 text-sm font-black text-center" />
      </div>

      <div {...getRootProps()} className="relative rounded-2xl bg-[#f8faff] border border-[#4a90e2]/10">
        <EditorContent editor={editor} className="min-h-[120px] w-full px-4 py-3 prose text-sm focus:outline-none" />
        <input {...getInputProps()} />
      </div>
      ...
      {!!attachments.length && (
        <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
          {attachments.map((attachment: any) => (
            <AttachmentStudio key={attachment.file.name} attachment={attachment} onRemove={() => removeAttachment(attachment.file.name)} />
          ))}
        </div>
      )}

      <div className="flex items-center justify-between pt-4 border-t border-border/40">
        <AddAttachmentsButton onFilesSelected={handleUpload} disabled={isUploading || attachments.length >= 10} label={t.add_media} />
        <LoadingButton onClick={onSubmit} loading={mutation.isPending} disabled={!isFormValid || isUploading} className="rounded-2xl px-6 bg-[#6ab344] text-white">
          <Sparkles className="size-3.5 mr-1.5" /> {t.publish_product}
        </LoadingButton>
      </div>
    </div>
  );
