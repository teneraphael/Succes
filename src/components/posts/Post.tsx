...
const extractInfo = (content: string) => {
  ...
  const priceMatch = content.match(
    /PRIX\s*:\s*([\d\s,._]+)\s*FCFA\s*\(([^)]+)\)/i,
  );

  const simplePriceMatch = content.match(
    /PRIX\s*:\s*([\d\s,._]+)\s*FCFA/i,
  );
  ...
  const whatsappMatch = content.match(
    /WHATSAPP\s*:\s*([^\n]+)/i,
  );
  ...
  return {
    ...
    price: priceMatch
      ...
      : simplePriceMatch
        ? simplePriceMatch[1].trim().replace(/\s/g, "")
        : null,
    ...
    cleanDescription: descMatch
      ? descMatch[1].trim()
      : content,

    whatsappNumber: whatsappMatch
      ? whatsappMatch[1].trim()
      : null,
...
/**
 * Tracking des interactions pour l'algorithme
 */
async function trackInteraction(
  postId: string,
  type: "VIEW" | "CHAT" | "FAVORITE",
) {
  try {
...
function InlineFollowButton({
  ...
  const { mutate, isPending } = useMutation({
    ...
    onMutate: async () => {
      ...
      const previousData =
        queryClient.getQueryData<FollowerInfo>(
          queryKey,
        );
      ...
      queryClient.setQueryData<FollowerInfo>(
        ...
        (old) => ({
          followers:
            (old?.followers || 0) +
            (old?.isFollowedByUser ? -1 : 1),

          isFollowedByUser:
            !old?.isFollowedByUser,
        }),
      );
    ...
    onSuccess: () => {
      toast({
        description:
          "Vous suivez désormais ce vendeur.",
        duration: 2000,
      });
    },
...
export default function Post({
  ...
  /**
   * Empêche plusieurs VIEW pour le même affichage
   */
  ...
  /**
   * Recherche de la variante correspondant
   * aux attributs sélectionnés
   */
  useEffect(() => {
    if (
      post.variants &&
      ...
      const matched = post.variants.find(
        (variant: any) => {
          const combo =
            variant.combinations as Record<
              string,
              string
            >;
  ...
  /**
   * 🔥 TRACKING INTELLIGENT DES VUES
   *
   * Le post doit être visible à au moins 60%
   * pendant 2 secondes.
   */
  useEffect(() => {
  ...
  const audioMedia = post.attachments.find(
    (m: any) => m.type === "AUDIO",
  );
  ...
  const handleWhatsApp = useCallback(async () => {
    if (!isAvailable) {
      toast({
        variant: "destructive",
        description: t.product_unavailable,
        duration: 2000,
      });
    ...
    if (!number) {
      toast({
        variant: "destructive",
        description: t.whatsapp_unavailable,
        duration: 2000,
      });
    ...
    /**
     * 🔥 Signal très fort pour l'algorithme :
     * l'utilisateur souhaite contacter le vendeur
     */
    trackInteraction(post.id, "CHAT");

    const choiceLabel = Object.entries(
      selectedAttributes,
    )
      .map(
        ([key, val]) =>
    ...
    lines.push(
      "Je suis interesse(e) par votre produit sur *DealCity* :",
    );
    ...
    lines.push(
      `Prix : *${currentPrice} FCFA*`,
    );

    if (choiceLabel) {
      lines.push(
        `Options choisies : *${choiceLabel}*`,
      );
    }
  ...
  return (
    <article
      ...
      className={cn(
        ...
        fullWidth
          ? "rounded-none"
          : "md:rounded-3xl max-w-xl mx-auto",
      )}
    >
      {/* En-tête */}
        ...
        <div className="flex flex-wrap items-center gap-3 min-w-0">
          <UserTooltip user={post.user}>
            <Link
              ...
              className="transition-transform active:scale-95 block shrink-0"
            >
              <UserAvatar
                avatarUrl={post.user.avatarUrl}
                className="ring-2 ring-[#4a90e2]/20 size-10"
              />
            </Link>
          </UserTooltip>
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 flex-wrap">
              <Link
                href={`/users/${post.user.username}`}
                className="font-extrabold text-sm tracking-tight text-foreground hover:text-[#4a90e2] transition-colors truncate"
              >
      ...
      <div className="px-5 pt-1">
        <div className="flex items-center justify-between pt-1 border-t border-border/40">
          ...
          >
            <svg
              viewBox="0 0 24 24"
              className="size-4 fill-current relative z-10 shrink-0"
