...
export default function PostMoreButton({ post, className }: PostMoreButtonProps) {
  ...
  const getSocialInfo = (url: string) => {
    ...
    if (lowerUrl.includes("tiktok.com")) return { label: "Voir sur TikTok", icon: <Music2 className="size-4 text-[#ff0050]" />, color: "text-[#ff0050]" };
    if (lowerUrl.includes("instagram.com")) return { label: "Voir sur Instagram", icon: <Instagram className="size-4 text-[#e1306c]" />, color: "text-[#e1306c]" };
    if (lowerUrl.includes("wa.me") || lowerUrl.includes("whatsapp.com")) return { label: "Contact WhatsApp", icon: <MessageCircle className="size-4 text-[#25d366]" />, color: "text-[#25d366]" };
    return { label: "Voir la boutique", icon: <Store className="size-4 text-[#4a90e2]" />, color: "text-[#4a90e2]" };
  };
  ...
  async function downloadMedia() {
    ...
    toast({ description: "Telechargement lance..." });
    for (const [i, media] of post.attachments.entries()) {
      ...
      } catch { toast({ variant: "destructive", description: "Erreur telechargement." }); }
    }
  }
  ...
  const itemClass = "flex items-center gap-2.5 px-3 py-2 rounded-xl cursor-pointer text-xs font-bold text-foreground hover:bg-muted transition-all";
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-60 p-1.5 shadow-xl border border-border/60 rounded-2xl bg-card">
        ...
        <DropdownMenuItem onClick={copyLink} className={itemClass}>
          <Copy className="size-4 text-muted-foreground" />
          Copier le lien
        </DropdownMenuItem>

        <DropdownMenuItem asChild>
          <a href={"/posts/" + post.id} target="_blank" rel="noopener noreferrer" className={itemClass}>
        ...
        {isOwner ? (
          ...
          <DropdownMenuItem onClick={handleReport} disabled={isReporting} className="flex items-center gap-2.5 px-3 py-2 rounded-xl cursor-pointer text-xs font-black text-amber-500 hover:bg-amber-500/8 transition-all disabled:opacity-50">
            ...
            {isReporting ? "Verification..." : "Signaler"}
          </DropdownMenuItem>
        )}
