...
export default function UserButton({ className, user: propUser }: UserButtonProps) {
  ...
  // ✅ Accès admin réservé à l'ID unique
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className={cn(
            "flex-none rounded-full transition-transform active:scale-95 outline-none",
            className,
          )}
        >
          <UserAvatar
            ...
            size={40}
            className="ring-2 ring-[#4a90e2]/20"
          />
        </button>
      </DropdownMenuTrigger>

      <DropdownMenuContent
        className="w-64 rounded-3xl p-2 shadow-xl border border-border/60 bg-card"
        align="end"
      >
        {/* ✅ En-tête utilisateur — style DealCity */}
        ...
        {/* ✅ Section admin — visible uniquement par l'administrateur */}
        {isAdmin && (
          <div>
            <DropdownMenuItem asChild className="rounded-xl py-2.5 cursor-pointer bg-[#4a90e2]/5 text-[#4a90e2] focus:bg-[#4a90e2]/10 focus:text-[#4a90e2]">
              <Link href="/admin/pioneers" className="flex items-center gap-2.5 w-full px-3">
                ...
                <span className="font-black uppercase italic text-xs">Repertoire Pionniers</span>
              </Link>
            </DropdownMenuItem>
        ...
        {/* ✅ Section vendeur ou devenir vendeur */}
        {user.isSeller ? (
          <DropdownMenuItem asChild className="rounded-xl py-2.5 cursor-pointer focus:bg-[#4a90e2]/5">
            <Link href="/seller/dashboard" className="flex items-center gap-2.5 w-full px-3">
              ...
              <span className="text-xs font-black uppercase tracking-tight">Tableau de Bord</span>
            </Link>
          </DropdownMenuItem>
        ) : (
        ...
        <DropdownMenuItem asChild className="rounded-xl py-2.5 cursor-pointer focus:bg-muted/50">
          <Link href="/settings" className="flex items-center gap-2.5 w-full px-3">
            ...
            <span className="text-xs font-bold">Parametres</span>
          </Link>
        </DropdownMenuItem>
