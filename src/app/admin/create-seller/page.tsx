...
import {
  ...
  Sparkles, MessageCircle, Facebook, Instagram, Music2, Phone, MapPin, Building, UserCheck, Copy, Check
} from "lucide-react";
...
const slides = [
  {
    label: "Onboarding Conciérgerie",
    sub: "Créez la vitrine de vos vendeurs en 1 clic",
    image: "https://images.pexels.com/photos/7621004/pexels-photo-7621004.jpeg",
  },
  {
    label: "Zéro friction",
    sub: "Le vendeur n'a rien à faire, vous gérez tout",
    image: "https://images.pexels.com/photos/6969967/pexels-photo-6969967.jpeg",
  },
  {
    label: "Boostez DealCity",
    sub: "Multipliez les marchands actifs instantanément",
...
export default function BecomeSellerPage() {
  ...
  // Fonction pour copier les accès prêts pour WhatsApp
  const copyCredentialsToClipboard = () => {
    if (!createdSellerInfo) return;
    const text = `🎉 Bienvenue sur DealCity !\n\nVotre boutique a été créée par notre service de conciergerie.\n\n👤 Identifiant : ${createdSellerInfo.username}\n🔑 Mot de passe temporaire : ${createdSellerInfo.tempPass}\n\nConnectez-vous dès maintenant pour gérer vos produits !`;
    
    navigator.clipboard.writeText(text);
    setCopied(true);
    toast({ description: "Informations copiées dans le presse-papier !" });
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        const result = (await createSellerDirectly(data)) as {
  success: boolean;
  error?: string;
...
if (!result.success) throw new Error(result.error || "Erreur lors de la création");
        ...
        toast({ description: "Boutique créée avec succès ! 🎉" });
        formElement.reset();
        setFormDataValues({ businessName: "", businessDomain: "", businessEmail: "", phoneNumber: "", city: "", neighborhood: "" });
        router.refresh();
      } catch (error: any) {
        toast({ variant: "destructive", description: error.message || "Une erreur est survenue." });
      }
    });
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex w-screen h-dvh overflow-hidden bg-[#f8fbff] dark:bg-[#09090b]">
      ...
      {/* ── COLONNE GAUCHE : Formulaire de création Admin ── */}
        ...
        <div className="flex-1 flex flex-col justify-center px-6 md:px-10 lg:px-12 py-8 max-w-lg mx-auto w-full">
          <div className="mb-7 space-y-2">
            <div className="flex items-end gap-2.5">
              ...
              <span className="text-xs font-black bg-amber-500 text-white px-2 py-0.5 rounded-lg uppercase tracking-wider">
                ADMIN CONCIERGE
              </span>
            </div>
            ...
            <p className="text-sm text-slate-500 dark:text-zinc-400">
              Enregistrez un vendeur à sa place pour éliminer tout frein
            </p>
          ...
          {/* Encart de succès interactif avec copie WhatsApp */}
          {createdSellerInfo && (
            <div className="mb-6 p-4 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-xl space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-emerald-800 dark:text-emerald-300 font-bold text-sm">
                  <UserCheck className="size-5 text-emerald-600" />
                  <span>Vendeur créé et activé !</span>
                </div>
                <Button 
                  type="button" 
                  size="sm" 
                  onClick={copyCredentialsToClipboard}
                  className="h-8 bg-emerald-600 hover:bg-emerald-700 text-white text-xs gap-1.5 font-bold"
                >
                  {copied ? <Check className="size-3.5" /> : <Copy className="size-3.5" />}
                  <span>{copied ? "Copié !" : "Copier pour WhatsApp"}</span>
                </Button>
              </div>
              <div className="bg-white dark:bg-zinc-900 p-2.5 rounded-lg border border-emerald-200 dark:border-emerald-800 text-xs font-mono space-y-1">
                <p>👤 <strong>Username :</strong> {createdSellerInfo.username}</p>
                <p>🔑 <strong>Mot de passe :</strong> {createdSellerInfo.tempPass}</p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="Nom de la boutique (ex: Chez Mama Krys)"
                  required
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <Textarea
                  ...
                  placeholder="Décrivez brièvement les produits..."
            ...
            <Button
              type="submit"
              disabled={isPending}
              className="w-full h-12 bg-gradient-to-r from-[#4a90e2] to-[#6ab344] hover:opacity-95 text-white font-black rounded-xl shadow-lg transition-all active:scale-[0.99] flex items-center justify-center gap-2 cursor-pointer"
            >
              {isPending ? (
                <>
                ...
                <>
                  <span>Créer la boutique vendeur</span>
                  <span>🚀</span>
                </>
              )}
            </Button>
            ...
            <p className="text-center text-[10px] text-slate-400 dark:text-zinc-500 font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 pb-4">
              ...
              <span>Mode Admin · Onboarding Conciergerie</span>
            </p>
          </form>
        </div>
      </div>

      {/* ── COLONNE DROITE : Visuels animés ── */}
