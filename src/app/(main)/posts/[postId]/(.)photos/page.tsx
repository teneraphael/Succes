...
export default function InterceptedPostPhotosPage({ params }: PageProps) {
  ...
  if (error || (!isLoading && !post)) {
    return (
      <div className="fixed inset-0 z-[110] bg-black flex flex-col items-center justify-center text-white gap-4 p-4">
        <div className="size-14 rounded-2xl bg-white/10 flex items-center justify-center">
          <ShoppingBag className="size-6 text-white/50" />
        </div>
        <p className="text-sm font-bold text-white/70">Erreur de chargement</p>
        <button
          onClick={() => router.back()}
    ...
    <LayoutGroup>
      ...
      >
        ...
        <motion.button
          ...
          onClick={() => router.back()}
          className="absolute top-5 left-5 z-[120] p-2.5 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-xl text-white transition-all active:scale-90 border border-white/10"
        >
          <X className="size-5" />
        </motion.button>

        {/* Galerie */}
        <div className="flex-1 overflow-y-auto bg-zinc-950 flex flex-col items-center snap-y snap-mandatory scrollbar-hide">
        ...
        <motion.div
          ...
          className="hidden md:flex w-[420px] bg-zinc-900 border-l border-white/8 flex-col p-7 overflow-y-auto gap-6"
        >
          {/* Vendeur */}
          ...
          {/* Badge DealCity */}
          <div className="flex items-center justify-center gap-2 py-3 border-t border-white/8">
